    ...
    if request.method == 'POST':
        ...
        try:
            ...
            cur.execute("""
                ...
                VALUES (%s, %s, %s, %s)
                RETURNING id_opef
            """, (date_opef, no_opef, nom_periode, id_annf))
...
#@app.route('/insertelnote', methods=['POST'])
#def insertelnote():
#    conn = None
#    cursor = None
#    try:
...
#        conn = psycopg2.connect(**DB_CONFIG)
#        cursor = conn.cursor()        
#        # الحصول على آخر عملية من جدول operation
#        cursor.execute("SELECT MAX(id_opef) FROM operation")
#        last_operation = cursor.fetchone()[0]
#        if not last_operation:
#            return jsonify({
#                'status': 'error',
#                'message': 'لا توجد عمليات مسجلة في الجدول operation'
#            }), 400
#        
#        # الحصول على آخر سنة دراسية من جدول ann_scolaire
#        cursor.execute("SELECT MAX(id_ann) FROM ann_scolaire")
#        last_academic_year = cursor.fetchone()[0]
#        if not last_academic_year:
#            return jsonify({
#                'status': 'error',
#                'message': 'لا توجد سنة دراسية مسجلة في الجدول ann_scolaire'
#            }), 400
#        
#        # الحصول على جميع هويات الطلاب
#        cursor.execute("SELECT id_el FROM eleves")
#        student_ids = [row[0] for row in cursor.fetchall()]
#        
#        # إدراج سجلات جديدة لكل طالب
#        inserted_count = 0
#        for student_id in student_ids:
#            cursor.execute("""
#                INSERT INTO eleve_note (id_el, id_ope, id_ann) 
#                VALUES (%s, %s, %s)
#            """, (student_id, last_operation, last_academic_year))
#            inserted_count += 1
#        
...
#        return jsonify({
#            'status': 'success',
#            'message': 'تم إدراج سجلات الدرجات بنجاح',
...
#    
#    finally:
#        # إغلاق الاتصال بغض النظر عن النتيجة
#        if cursor:
#            cursor.close()
#        if conn:
#            conn.close()     

@app.route('/exement/update/<id>', methods=['POST'])
def update_exement(id):
    if not session.get('logged_in'):
        return redirect(url_for('login'))    
    if 'annee_info' not in session:
        flash('الرجاء اختيار سنة دراسية أولاً', 'warning')
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            cur.execute("""
                ...
                WHERE id_opef=%s
            """, (date_opef, no_opef, nom_periode, id_annf, id))
...
def select_operation():
    ...
    if request.method == 'GET':
        ...
        try:
            conn = get_db_connection()
            cur = conn.cursor()
            cur.execute('SELECT id_opef, nom_periode FROM operation ORDER BY id_opef')
...
def handle_notes_template(template_name):
    ...
    try:
        ...
        cur.execute('''
            SELECT id_el, id_ma, nolesel, note_compo, moy_coeff
        ...
        if request.method == 'POST':
            try:
                ...
                inserreng()
                #updatemanuel()
                #flash('تم تحديث جميع الدرجات والمعدلات بنجاح', 'success')
                return redirect(request.url)               
...
def modifier_notes():
    try:
        ...
        cur.execute("""
            ...
            RETURNING id_ne_cl
        """, (
            data['id_el'], data['nolesel'], data['id_ma'], 
            data['id_ope'], data['note_compo'], 
            data['moyen'], data['moy_coeff'], data['id_ne_cl']
        ))
        ...
        for row in rows:
            id_ne_cl, id_el, id_ope, note_compo, nolesel, coef_val, moyen_val = row
            
            # حساب القيم
            moy_coeff = round(moyen_val * coef_val, 2)
